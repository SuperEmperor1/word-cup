...
协议 (严格无泄漏):
  GBM 训练:           date < train_end
  融合权重/温度调参:   train_end <= date < valid_end
  测试:               date >= valid_end
DC 模型在测试期内按季度重拟合, 每次仅用当时已发生的比赛。
指标: RPS / Brier / 对数损失 / 命中率, 并与 基准(频率先验) 和 纯Elo 对比。
"""
...
def walk_forward(feat: pd.DataFrame,
    ...
    ens = Ensemble()
    ens.fit_gbm(feat[tr], y[tr])

    # 验证集: DC 拟合至 train_end
    ens.dc = DixonColes().fit(feat[feat["date"] < train_end])
    p_dc_va = ens.dc_probs_rows(feat[va])
    p_ml_va = ens.gbm_probs(feat[va])
    ens.tune(p_dc_va, p_ml_va, y[va])
    ...
    return {
        ...
        "weight_dc": ens.weight, "temperature": ens.temp,
